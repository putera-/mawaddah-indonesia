...
export class PhysicalCharsService {
  ...
  async create(id: string, data: Prisma.Physic_characterCreateInput) {
    return this.prisma.physic_character.create({
  ...
  async findOne(userId: string, id: string) {
    const data = await this.prisma.physic_character.findFirst({ where: { id, userId, deleted: false }, select });
    if (!data) {
      // Check if the education record exists for any user
      const phyExist = await this.prisma.physic_character.findUnique({ where: { id, deleted: false } });
  ...
  async update(userId: string, id: string, data: UpdatePhysicalCharDto) {
    ...
    return this.prisma.physic_character.update({
      where: { id: phyId.id },
