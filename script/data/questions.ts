...
export async function questions(
    ...
    const [questions_data]: any[] = await old_db.execute(
    ...
    for (const _question of questions_data) {
        ...
        let question_value =
            _question.tanya.trim().charAt(0).toUpperCase() +
            _question.tanya.trim().slice(1);
        let question = question_list.find((q) => q.question == question_value);
        if (question_value == '') {
